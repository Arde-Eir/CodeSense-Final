...
import { countUniqueCompletedQuests, type CompletedMissionRow } from '@/components/dashboard/dashboardMetrics'
import {
  AnnouncementsModal,
  NotificationBell,
...
export const HomeDashboard: React.FC = () => {
  ...
  const [searchResults, setSearchResults] = useState<SearchResults>({ actions: [], players: [], quests: [], reports: [] })
  ...
  const runSearch = useCallback(async (q: string) => {
    ...
    try {
      const { data: players } = await supabase.from('users').select('id, playername, totalxp, currentlevel').eq('isactive', true).eq('is_banned', false).ilike('playername', `%${q}%`).limit(5)
      ...
      const questMap = new Map<string, SearchQuest>()
      ;[...(t.data ?? []), ...(ph.data ?? []), ...(d.data ?? [])].forEach((item: SearchQuest) => questMap.set(item.id, item))
      let reportsData: SearchReport[] = []
      if (user && !isGuest) {
        ...
        const rm = new Map<string, SearchReport>()
        ;[...(byMode.data ?? []), ...(byCode.data ?? [])].forEach((report: SearchReport) => rm.set(report.id, report))
  ...
  const fetchLeaderboard = useCallback(async () => {
    if (!user) return
    try {
      const { data: lb } = await supabase.from('users').select('id, playername, totalxp, currentlevel').eq('isactive', true).eq('is_banned', false).order('totalxp', { ascending: false }).limit(10)
      if (lb) {
        ...
        if (myPos !== -1) { setMyRank(myPos + 1) }
          ...
          const { count } = await supabase.from('users').select('*', { count: 'exact', head: true }).eq('isactive', true).eq('is_banned', false).gt('totalxp', user.totalXP ?? 0)
          setMyRank((count ?? 0) + 1)
        }
      }
  ...
  const fetchStats = useCallback(async () => {
    ...
    try {
      ...
      const { data: progressRows } = await supabase
        ...
        .eq('userid', user.id)
      if (profile) setStats({ sandboxRuns: profile.sandbox_runs ?? 0, questsCompleted: countUniqueCompletedQuests((progressRows ?? []) as CompletedMissionRow[]), xpToNextLevel: getXPToNextLevel(profile.totalxp ?? 0), levelProgress: getLevelProgress(profile.totalxp ?? 0) })
